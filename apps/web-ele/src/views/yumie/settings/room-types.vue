...
const rules: FormRules = {
  ...
  interval: [
    { message: '請輸入時間區間', required: true, trigger: 'blur' },
  ],
...
async function save(): Promise<void> {
  ...
  try {
    ...
    if (editingId.value) {
      await roomTypesApi.update(editingId.value, payload as RoomTypeUpdateInput);
    ...
    <ElCard>
      <template #header>
        <div class="flex items-center justify-between">
          <span>房型 — {{ hotelStore.currentHotelMeta?.hotelName ?? currentHotelId }}</span>
      ...
      <ElTable v-loading="loading" :data="rows" border row-key="id">
        ...
        <ElTableColumn label="同步天數" prop="inventorySyncToHotelDocDays" width="120" align="center" />
        <ElTableColumn label="時間區間" prop="interval" width="120" align="center" />
        <ElTableColumn label="PMS code" prop="pmsRoomTypeCode" width="140" />
        <ElTableColumn label="Fly Kiosk Room Type ID" prop="flyKioskRoomTypeId" width="200" />
        <ElTableColumn label="關聯方案" min-width="280">
          <template #default="{ row }">
            <ElSpace v-if="row.associatePlansId?.length" wrap>
              <ElTag v-for="planId in row.associatePlansId" :key="planId" type="info">
                {{ planLabel(planId) }}
              </ElTag>
            </ElSpace>
        ...
        <ElTableColumn label="操作" width="160" align="center" fixed="right">
          <template #default="{ row }">
            <ElButton size="small" @click="openEdit(row as RoomType)">編輯</ElButton>
            <ElPopconfirm
              title="確認刪除？"
    ...
    >
      ...
      >
        ...
        <ElFormItem label="同步天數" prop="inventorySyncToHotelDocDays">
          <ElInputNumber v-model="form.inventorySyncToHotelDocDays" :min="1" :max="30" />
        </ElFormItem>
        <ElFormItem label="時間區間（分鐘）" prop="interval">
          <ElInputNumber v-model="form.interval" :min="5" :max="120" :step="5" />
        </ElFormItem>
